...
function canonicalQuery(parameters: Record<string, string>): string {
  return Object.entries(parameters)
    .map(
      ([key, value]) =>
        [encodeRfc3986(key), encodeRfc3986(value)] as const,
    )
...
export class R2StorageService {
  ...
  constructor() {
    ...
    const configuredExpiry = Number(
      process.env.R2_PRESIGNED_URL_EXPIRES_SECONDS,
    );
  ...
  createResumeUploadUrl(
    ...
    if (!expectedExtension) {
      throw new Error(
        "Unsupported resume type. Only PDF, DOC, and DOCX files are accepted.",
      );
    }
    ...
    if (
      ...
      input.size <= 0 ||
      input.size > MAX_RESUME_SIZE
    ) {
      throw new Error(
        "Resume must be between 1 byte and 10 MB.",
      );
    }
  ...
  private isValidResumeKey(
    uploadId: string,
    storageKey: string,
  ): boolean {
    return new RegExp(
      `^resumes/${uploadId}\\.(pdf|doc|docx)$`,
      "i",
    ).test(storageKey);
  }
  ...
  private createPresignedUrl(
    ...
    const canonicalUri =
      `/${encodePath(this.bucket)}/${encodePath(storageKey)}`;

    const now = new Date();

    const { shortDate, amzDate } =
      formatAmzDate(now);
